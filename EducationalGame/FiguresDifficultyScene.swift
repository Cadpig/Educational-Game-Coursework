//
//  FiguresDifficultyScene.swift
//  EducationalGame
//
//  Created by Владислав Тихонов on 22.04.2021.
//  Copyright © 2021 Владислав Тихонов. All rights reserved.
//

import SpriteKit

class FiguresDifficultyScene: SKScene {
    ...
    fileprivate func spawnButtons(){
        let buttonEasy = Button.populate(name: "difButton1", zPos: 1, textureName: "btnfigdif1", at: CGPoint(x: self.frame.midX, y: self.size.height * 3 / 5), scale: 0.5)
        ...
        let buttonMedium = Button.populate(name: "difButton2", zPos: 1, textureName: "btnfigdif2", at: CGPoint(x: self.frame.midX, y: self.size.height * 2 / 5), scale: 0.5)
        ...
        let buttonHard = Button.populate(name: "difButton3", zPos: 1, textureName: "btnfigdif3", at: CGPoint(x: self.frame.midX, y: self.size.height * 1 / 5), scale: 0.5)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if node.name == "difButton1"{
            ...
            let gameScene = FiguresEasyMenuScene(size: self.size)
        ...
        if node.name == "difButton2"{
            let transition = SKTransition.crossFade(withDuration: 1)
            let gameScene = FiguresMediumMenuScene(size: self.size)
            gameScene.scaleMode = .aspectFill
            self.scene?.view?.presentScene(gameScene, transition: transition)
        }
        ...
        if node.name == "difButton3"{
            let transition = SKTransition.crossFade(withDuration: 1)
            let gameScene = FiguresHardMenuScene(size: self.size)
            gameScene.scaleMode = .aspectFill
            self.scene?.view?.presentScene(gameScene, transition: transition)
        }
