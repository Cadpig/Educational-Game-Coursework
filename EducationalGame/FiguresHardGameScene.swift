...
class FiguresHardGameScene: SKScene {
    ...
    func spawnFigureName(){
        let distribution = GKRandomDistribution(lowestValue: 0, highestValue: 5)
        ...
        figureLabel.position = CGPoint(x: 500, y: self.size.height - 100)
    ...
    func spawnFigure(){
        ...
        let spawnFigureAction = SKAction.run{
            let figure = Figure.populate(at:CGPoint(x: 0, y: 100), range: 6)
